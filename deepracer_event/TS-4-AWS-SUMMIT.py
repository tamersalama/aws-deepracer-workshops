...
class RewardEvaluator:
    ...
    MAX_STEERING_ANGLE = 30 #granurality is 5
    ...
    # down.
    SAFE_HORIZON_DISTANCE = 2 # 0.8  # meters, able to fully stop. See ANGLE_IS_CURVE.
    ...
    def evaluate(self):
        ...
        try:
            ...
            if self.all_wheels_on_track == False or self.is_reversed == True or (self.speed < (0.1 * self.MAX_SPEED)):
                self.log_feature("all_wheels_on_track or is_reversed issue or too slow issue")
            ...
            # just keep the car in centre
            if self.is_near_center():
                self.log_feature("is_near_center")
                # 
                result_reward = result_reward + float(self.REWARD_MAX * 0.6)
